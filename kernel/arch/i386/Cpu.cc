...
union CpuExtFeatures2_ {
  uint32_t raw_data;
  struct {
    uint32_t LAHF : 1; // LAHF and SAHF instructions available when the IA-32e mode is enabled and
                       // the processor is operating in the 64-bit sub-mode.
...
void cpuVendorId(char result[13], uint32_t &funcMax)
{
  ...
  // as a side effect the maximum number of functions to query cpuid with is provided.
  funcMax = res[0];
}
...
bool Cpu::init()
{
  ...
  if (funcMaxExt >= 0x80000002 && funcMaxExt >= 0x80000003 && funcMaxExt >= 0x80000004) {
...
{
  uint32_t flags;
  __asm__("pushfl;"    // Push eflags on stack.
          "pop %%eax;" // Save to EAX.
          : "=a"(flags));
  return flags;
}
